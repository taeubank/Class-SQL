...
------------------------------------------------------------
-- PARAMETERS (keep these at the top; DO NOT put GO after)
------------------------------------------------------------
DECLARE
...
------------------------------------------------------------
-- Optional wipe (facts + patients/insurance)
------------------------------------------------------------
IF @WipeFacts = 1
BEGIN
...
------------------------------------------------------------
-- Create PatientInsurance (primary)
------------------------------------------------------------
...
------------------------------------------------------------
-- Appointments (random count per patient)
------------------------------------------------------------
...
------------------------------------------------------------
-- EncounterProcedure (1..@MaxProcsPerEnc)
------------------------------------------------------------
DECLARE @ProcCount INT = (SELECT COUNT(*) FROM dbo.ProcedureCode);
...
------------------------------------------------------------
-- Prescriptions (~@PctRx of encounters)
------------------------------------------------------------
...
------------------------------------------------------------
-- Payments (payer + optional patient)
------------------------------------------------------------
;WITH totals AS (
    SELECT c.ClaimID, SUM(cl.Quantity * cl.UnitCharge) AS TotalCharges
    FROM dbo.Claim c
    JOIN dbo.ClaimLine cl ON cl.ClaimID = c.ClaimID
    GROUP BY c.ClaimID
),
payable AS (
    SELECT ClaimID, TotalCharges,
           ABS(CHECKSUM(CONCAT('PAY',@Seed,ClaimID))) % 100 AS R
    FROM totals
)
